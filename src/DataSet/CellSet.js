class CellSet extends DataSetComponent {
  ...
  #buildRemoteCellsQuery(tuplesToQuery, tuplesFields, cellsAxisItemsToFetch){
    const queryModel = this.getQueryModel();
    const rowsAxis = queryModel.getRowsAxis().getItems();
    const columnsAxis = queryModel.getColumnsAxis().getItems();
    const filterAxis = queryModel.getFiltersAxis().getItems();
    const filters = filterAxis.filter((item) => { return item.filter; }).map((item) => {
    ...
    const axes = {
      rows: rowsAxis.map((item) => { return { field: item.columnName, derivation: item.derivation || null }; }),
      columns: columnsAxis.map((item) => { return { field: item.columnName, derivation: item.derivation || null }; }),
      measures: (cellsAxisItemsToFetch || []).map((item) => {
  ...
  async #executeCellsQuery(
    ...
    if (isRemote && datasource.getManagedConnection().fetchCells) {
      ...
      const dateRange = { type: 'single', date: new Date().toISOString().slice(0, 10) };
      const connection = await this.getManagedConnection();
      const apiResponse = await connection.fetchCells(dateRange, query);
      const resultSet = this.#remoteCellsResponseToResultSet(apiResponse, cellsAxisItemsToFetch, colCount);
      return resultSet;
    }
